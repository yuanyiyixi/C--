...
:QLabel(parent)
{
	...
	QPalette plt;
	//setAutoFillBackground(true);			//文本显示区域背景颜色
	//plt = palette();
	//plt.setColor(QPalette::Background, Qt::yellow);
	//setPalette(plt);

	text = new QLabel(parent);				//创建文字显示label
	plt = text->palette();					//设置文字颜色
	text_color.setRgb(0, 0, 0xff, 255);		//颜色值(R,G,B,A),A透明度, 255表示没有透明
	plt.setColor(QPalette::WindowText, text_color);
	text->setPalette(plt);
	text_family = "AR PL UKai CN";			//设置字体,楷体
	
	text_size = height() * 0.4;				//初始化字号
	//qDebug() << "text_size=" << text_size;
	text->setFont(QFont(QString::fromUtf8(text_family), text_size));	//字体字号
	texttimer = new QTimer(this);					//定时器
	texttimer->start(300);							//设置为10毫秒
	connect(texttimer, SIGNAL(timeout()), this, SLOT(dynamic_displaytext()));
	i = width();									//初始化I值,用于动态显示
	isshow = true;			//显示状态标志
	isdynamic = true;		//动态显示标志
        smoothness = 8;			//默认平滑度8
}

void Textarea::setmsg(QString msg)		//设置文字内容
{
	//qDebug() << "size=" << msg.size();
        text->setText(msg);
	//qDebug() << "fontsize=" << text->fontMetrics().width(text->text());
        textwidth = text->fontMetrics().width(text->text()) * 1;
	//qDebug() << "textwidth=" << textwidth;
}

void Textarea::setcolor(int r, int g, int b, int a)		//rgba方式更改颜色
{
	QPalette plt;
...
void Textarea::setcolor(int c, int m, int y, int k, int a) //cmyk方式更改颜色
{
	QPalette plt;
...
void Textarea::setfontsize(int size)	//设置字号
{
...
void Textarea::setfontfamily(char *fontfamily)		//设置字体
{
...
void Textarea::dynamic_displaytext()			//动态显示字幕
{
...
void Textarea::change_display_mode()			//切换静态/动态显示模式
{
...
void Textarea::change_show_hide()				//显示/隐藏
{
	if (isshow)
		text->hide();
	else
